...
from almacen.models import Proyectos, Proveedor, GeneralDetalle, Materiales, UnidadMedida, IngresoCabecera, \
    IngresoDetalle, IngresoConformidad, Ubigeo, Colaborador, Clientes, ColaboradoresCargo
...
def post_ingresos(request):
    ingresoCabecera = IngresoCabecera.objects.all().select_related('id_proyecto', 'vc_codEstado', 'id_proveedor',
                                                                   'vc_codTipoComprobante', 'vc_codCondicionPago')
...
def post_nuevo(request):
    ...
    return render(request, 'ingresos/nuevo.html',
                  {'proyectos': proyectos, 'recepcion': recepcion, 'tipocomprobante': comprobante
...
def buscarMaterial(request):
    ...
    material = Materiales.objects.filter(vc_descripcion__icontains=descripcion, id_codEstado=idTipo).values(
        'vc_descripcion', 'id_materiales', 'vc_codigo')
...
def buscarCodMaterial(request):
    ...
    material = Materiales.objects.filter(vc_descripcion=descripcion).values('id_unidadMedida', 'vc_codigo',
                                                                            'id_materiales')
...
def buscarDescMaterial(request):
    if request.POST['codigo']:
        material = Materiales.objects.filter(vc_codigo=request.POST['codigo']).values('id_unidadMedida',
                                                                                      'vc_descripcion', 'id_materiales',
                                                                                      'vc_codEstado')
    else:
        material = Materiales.objects.filter(Q(vc_codigo=request.POST['codigo']) | Q(vc_codigo=None)).values(
            'id_unidadMedida', 'vc_descripcion', 'id_materiales', 'vc_codEstado')
    list_result = [value for value in material]
    if list_result:
        medidas = UnidadMedida.objects.filter(id_unidadMedida=list_result[0]['id_unidadMedida']).get()
        list_result.append(medidas.nu_valor)
...
def guardar(request):
    ...
    cabecera = IngresoCabecera(vc_ocRayro=x.getlist('num_orden')[0], vc_codRepcion=recepcion, id_proyecto=proyecto,
                               vc_codEstado=estado, dt_emision=date, id_proveedor=proveedor,
                               vc_codTipoComprobante=tipoComprobante,
                               vc_serieComprobante=x.getlist('serie')[0],
                               vc_numComprobante=x.getlist('num_comprobante')[0],
                               vc_codCondicionPago=condPago, fl_importe=x.getlist('importe_comprobante')[0],
                               fl_igv=x.getlist('igv_comprobante')[0],
    ...
    for a in array:
        ...
        for c in b:
            ...
            detalle = IngresoDetalle(id_ingresoCabecera=cabecera, vc_codDireccionGasto=direccionGasto,
                                     id_materiales=material, nu_cantidad=cdict.get('cantidad_detalle'),
                                     nu_totalIngreso=cdict.get('total_detalle'),
                                     nu_precioUnitario=cdict.get('precio_detalle'),
                                     nu_Importe=cdict.get('importe_detalle'),
                                     nu_igv=cdict.get('igv_detalle'), nu_subTotal=cdict.get('subtotal_detalle'),
                                     dt_crea=datetime.now(), b_flagInactivo=0,
...
def editar(request, idIngreso, idVista):
    ...
    detalles = IngresoDetalle.objects.filter(id_ingresoCabecera=idIngreso, b_flagInactivo=0).select_related(
        'vc_codDireccionGasto', 'id_materiales')
    return render(request, 'ingresos/editar.html',
                  {'ingresoCabecera': ingresoCabecera, 'proyectos': proyectos, 'recepcion': recepcion,
                   'tipocomprobante': comprobante
                      , 'condicionpago': condicion, 'direcciongasto': gasto, 'estado': estado, 'detalles': detalles,
                   'idVista': idVista, 'idModal': idModal})
...
def editarIngreso(request):
    ...
    cabecera = IngresoCabecera.objects.filter(id_ingresoCabecera=request.POST['idEditarIngreso']).update(
        vc_ocRayro=data.getlist('num_orden')[0], vc_codRepcion=recepcion, id_proyecto=proyecto,
        ...
        vc_codCondicionPago=condPago, fl_importe=data.getlist('importe_comprobante')[0],
        fl_igv=data.getlist('igv_comprobante')[0],
    ...
    for a in array:
        ...
        for c in b:
            ...
            material = Materiales.objects.get(id_materiales=cdict.get('id_materiales'))
            if idIngresoDetalle:
                actDetalle = IngresoDetalle.objects.filter(id_ingresoDetalle=cdict.get('id_detalle')).update(
                    vc_codDireccionGasto=cdict.get('direccion_gasto'),
                    id_materiales=material, nu_cantidad=cdict.get('cantidad_detalle'),
                    nu_totalIngreso=cdict.get('total_detalle'),
                ...
                detalle = IngresoDetalle(id_ingresoCabecera=ingreso, vc_codDireccionGasto=direccionGasto,
                                         ...
                                         nu_Importe=cdict.get('importe_detalle'),
                                         nu_igv=cdict.get('igv_detalle'), nu_subTotal=cdict.get('subtotal_detalle'),
                                         dt_crea=datetime.now(), bl_conformidadAdm=0, bl_conformidadOC=0,
                                         b_flagInactivo=0)
                detalle.save()
    ...
    if not detalleActivo:
        IngresoCabecera.objects.filter(id_ingresoCabecera=request.POST['idEditarIngreso']).update(fl_total=0, fl_igv=0,
                                                                                                  fl_importe=0)
    global modal
    modal = 2
...
def conformidad(request, idIngreso):
    ...
    print(obConformidad)
    return render(request, 'ingresos/conformidad.html',
                  {'detalles': detalles, 'estado': estado, 'idIngreso': idIngreso, 'idConformidad': idConformidad,
                   'importeConformidad': importeConformidad, 'obConformidad': obConformidad,
                   'estadoConformidad': estadoConformidad, 'idModal': idModal})
...
def guardarConformidad(request):
    ...
    if request.POST['idConformidadEdit']:
        conformidad = IngresoConformidad.objects.filter(id_ingresoConformidad=request.POST['idConformidadEdit']).update(
            fl_importeConformidad=request.POST['total_compra'],
            tx_observacionConformidad=request.POST['observaciones'], vc_codConformidad=codConformidad, b_flagInactivo=0,
        ...
        for a in array:
            ...
            for c in b:
                cdict = ast.literal_eval(c)
                actDetalle = IngresoDetalle.objects.filter(id_ingresoDetalle=cdict.get('id_detalle')).update(
                    bl_conformidadOC=cdict.get('check1_detalle'),
                    bl_conformidadAdm=cdict.get('check2_detalle'))
    else:
        conformidad = IngresoConformidad(id_ingresoCabecera=ingreso, fl_importeConformidad=request.POST['total_compra'],
                                         tx_observacionConformidad=request.POST['observaciones'],
                                         dt_fecConformidad=datetime.now(),
