...
def buscarDescMaterial(request):
	if request.POST['codigo']:
		material = Materiales.objects.filter(vc_codigo= request.POST['codigo'] ).values('id_unidadMedida', 'vc_descripcion' , 'id_materiales', 'vc_codEstado')
	else:
		material = Materiales.objects.filter(Q (vc_codigo= request.POST['codigo']) | Q (vc_codigo= None) ).values('id_unidadMedida', 'vc_descripcion' , 'id_materiales', 'vc_codEstado')
	list_result = [value for value in material]
	if list_result:
		medidas = UnidadMedida.objects.filter(id_unidadMedida=list_result[0]['id_unidadMedida']).get()
		list_result.append(medidas.nu_valor)
...
def guardarConformidad(request):
	...
	if request.POST['idConformidadEdit']:
		conformidad = IngresoConformidad.objects.filter(id_ingresoConformidad=request.POST['idConformidadEdit']).update(fl_importeConformidad=request.POST['total_compra'] ,
